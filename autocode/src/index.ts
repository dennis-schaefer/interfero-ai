...
agent
  ...
  .action(async (opts: { issue: number; dryRun: boolean }) => {
    ...
    console.error(`[autocode] planning phase…`);
    await runPlanningPhase(prd, order);
    ...
    try {
      for (const issue of order) {
        ...
        await runImplementationPhase(issue);
